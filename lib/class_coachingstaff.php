...
class Coachingstaff 
{
    ...
    {
        /* 
            Creates a coaching staff object with up-to-date fields/stats reflecting the coaching staff's participation in every match.
        */

        $this->coachingstaff_id = $sid;
        $this->setStats(false, false, false);
        global $coachingstaff;
        $this->icon = PLAYER_ICONS.'/'.$coachingstaff[$this->name]['icon'].'.png';
    }
    
    public function setStats($node, $node_id, $setAvg = false)
    {
        foreach (Stats::getAllStats(T_OBJ_COACHINGSTAFF, $this->coachingstaff_id, $node, $node_id, $setAvg) as $key => $val) {
            $this->$key = $val;
        }
    }
    ...
    public function getStats($type, $type_id) 
    {
        ...
        $query = "SELECT ".implode(',', array_map(create_function('$f', 'return "SUM($f) AS \'$f\'";'), $fields))." FROM match_data WHERE f_player_id = $this->coachingstaff_id AND ".$STATS_TRANS[$type].'='.$type_id;
    ...
    {
        $query = "SELECT skills FROM game_data_coachingstaff WHERE coachingstaff_id = $this->coachingstaff_id";
        $result = mysql_query($query);
    ...
    public function getHireHistory($obj, $obj_id, $node, $node_id)
    {
        ...
        */
        ...
        $query = "SELECT DISTINCT f_match_id, f_team_id FROM match_data, matches WHERE 
            f_match_id = match_id 
            AND f_player_id = $this->coachingstaff_id 
    ...
    public function rmMatchEntry($match_id, $team_id = false)
    {
        ...
        */
        ...
        $result = mysql_query("SELECT * FROM match_data WHERE f_player_id = $this->coachingstaff_id AND f_match_id = $match_id");
        $CNT = mysql_num_rows($result);
        $query = "DELETE FROM match_data WHERE f_player_id = $this->coachingstaff_id AND f_match_id = $match_id" . (($team_id) ? " AND f_team_id = $team_id" : '');
        mysql_query($query);
        if ($CNT > 0) {
            $trid = get_alt_col('matches', 'match_id', $match_id, 'f_tour_id');
            $q = "SELECT syncMVplayer($this->coachingstaff_id, $trid)";
            mysql_query($q);
        }
        return true;
    }
    
    public static function getCoachingStaff($obj, $obj_id, $node, $node_id)
    {
        /*
            Returns an array of star objs for each (depending on arguments) star player.
        */
        
        global $coachingstaff, $STATS_TRANS;
        $coachingstaffObjs = array();
        
        if (!($obj || $node)) {
            $coachingstaffObjs = array_map(create_function('$s', 'return (new Star($s[\'id\']));'), $coachingstaff);
        }
        else {
            $query = "SELECT DISTINCT f_player_id FROM match_data, matches WHERE 
                f_match_id = match_id AND f_player_id <= ".ID_COACHINGSTAFF_BEGIN." 
                ".(($obj)  ? ' AND '.$STATS_TRANS[$obj]. " = $obj_id "  : '').'
                '.(($node) ? ' AND '.$STATS_TRANS[$node]." = $node_id " : '');
            if (($result = mysql_query($query)) && mysql_num_rows($result) > 0) {
                while ($row = mysql_fetch_assoc($result)) {
                    array_push($coachingstaffObjs, new Star($row['f_player_id']));
                }
            }
        }
        
        return $coachingstaffObjs;
    }
    
    public static function exists($id)
    {
        return get_alt_col('game_data_coachingstaff', 'coachingstaff_id', $id, 'coachingstaff_id');
    }
}
