...
public static function getRecentEvents($event, $node, $node_id, $N) 
{
    ...
    case self::T_PLAYER_SKILLS:
        $query = "SELECT player_id AS 'pid', game_data_skills.skill_id AS 'skill_id', game_data_skills.cat AS 'cat', game_data_skills.name AS 'skill_name', CONCAT(game_data_skills.name,' (',game_data_skills.cat,')') AS 'skill', $_COMMON_COLS__T_PLAYER, players_skills.id FROM players, mv_players, players_skills, game_data_skills WHERE 
        ...
        GROUP BY player_id, f_skill_id, players_skills.id ORDER BY players_skills.id DESC LIMIT $N";
		//print($query);
        $result = mysql_query($query);
        while ($row = mysql_fetch_object($result)) {
            $events[] = $row;
